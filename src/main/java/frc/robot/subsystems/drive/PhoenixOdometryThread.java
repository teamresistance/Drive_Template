...
public class PhoenixOdometryThread extends Thread {
  ...
  public Queue<Double> registerSignal(StatusSignal<Angle> signal) {
    ...
    try {
      ...
      signalsLock.unlock();
      SwerveDriveReal.ODOMETRY_LOCK.unlock();
    }
    return queue;
  }
    ...
    while (true) {
      ...
      try {
        if (IS_CANFD && phoenixSignals.length > 0) {
          BaseStatusSignal.waitForAll(2.0 / SwerveDriveReal.ODOMETRY_FREQUENCY, phoenixSignals);
        } else {
          // "waitForAll" does not support blocking on multiple signals with a bus
          // that is not CAN FD, regardless of Pro licensing. No reasoning for this
          // behavior is provided by the documentation.
          Thread.sleep((long) (1000.0 / SwerveDriveReal.ODOMETRY_FREQUENCY));
